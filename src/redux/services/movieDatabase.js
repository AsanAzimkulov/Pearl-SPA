...
export const movieCoreApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getMovieDetails: builder.query({
      query: ({ movieId }) => `/get-overview-details?tconst=tt${movieId}`,
    }),
    getMovieImg: builder.query({
      query: ({ movieId }) => `/get-images?tconst=tt${movieId}&limit=2`,
    }),
    getMovieCast: builder.query({
      query: ({ movieId }) => `/get-full-credits?tconst=${movieId}`,
    }),
